...
### ۰٫۱ آیا سرور به GitHub می‌رسد؟

در Terminal سی‌پنل:

```bash
for u in https://repo.packagist.org/packages.json \
         https://codeload.github.com/laravel/framework/tar.gz/refs/tags/v12.0.0 ; do
...
| نتیجه | یعنی |
| --- | --- |
| هر دو `200` | **مسیر الف** — `composer install` روی سرور کار می‌کند |
| یکی `403` یا `000` | **مسیر ب** — پوشه `vendor` را آماده می‌فرستیم |
...
### مسیر الف — با git (اگر مرحله ۰٫۱ سبز بود)
...
### مسیر ب — با آپلود فایل

اگر GitHub در دسترس نبود، به من بگویید تا یک بسته کامل (شامل `vendor`) بسازم.
آن را با **File Manager** سی‌پنل در پوشه خانه آپلود و همان‌جا Extract می‌کنید.
