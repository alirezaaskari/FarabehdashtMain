...
### پیامک اعلان‌های مهم (بخش ۱۸-۲)

پیش‌فرض خاموش است. برای روشن‌کردن:

1. در پنل ملی‌پیامک یک الگوی خدماتی تازه با دو متغیر ثبت کنید و منتظر تأییدش بمانید:
   ```
   فرابهداشت: {0}
   {1}
   ```
   `{0}` عنوان خبر است و `{1}` پیوند آن.
2. در `.env` هاست:
   ```dotenv
   MELIPAYAMAK_PATTERN_NOTICE=<شناسه الگوی تأییدشده>
   FBH_SMS_NOTICES=true
   ```
3. `bash scripts/deploy.sh`.

با درایور `melipayamak` (خط عمومی) الگو لازم نیست، ولی پیامک به گوشی‌هایی که
تبلیغات را بسته‌اند نمی‌رسد. هر کاربر روزی حداکثر ۳ پیامک می‌گیرد و از ۲۲ تا ۸
هیچ پیامکی نمی‌رود.
...
### ورود مدیر با ایمیل و رمز عبور (اختیاری)

کنار ورود پیامکی، مدیر می‌تواند از «ورود مدیر با ایمیل» در صفحه ورود
(`/login/email`) با ایمیل و رمز وارد شود. تنظیم ایمیل (SMTP) لازم نیست. ایمیل
و رمز فقط از خط فرمان ثبت می‌شوند:

```bash
/usr/local/bin/ea-php84 artisan fbh:make-admin 09xxxxxxxxx --email=you@example.com --password
```

رمز پنهان و دو بار پرسیده می‌شود (دست‌کم ۱۲ نویسه) و در تاریخچه شل نمی‌ماند.
برای عوض‌کردن رمز، همین دستور را دوباره بزنید. پس از ۵ رمز نادرست، آن ایمیل
۱۵ دقیقه قفل می‌شود. اگر مدیر ایمیلش را در «حساب من» عوض کند، ورود با ایمیل تا
اجرای دوباره دستور با `--email` بسته می‌ماند.
...
| پنل مدیریت کاملاً بی‌استایل | `filament:assets` اجرا نشده: `bash scripts/deploy.sh` را دوباره بزنید. بررسی: `ls public/css/filament/filament/app.css` |
| پنل استایل دارد ولی فرم‌ها و فهرست‌ها بی‌فاصله‌اند | `public/build/assets/theme-*.css` نیست یا قدیمی است — `git pull` کامل انجام نشده |
