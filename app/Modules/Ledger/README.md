...
موجودی کیف پول برای خرید داخل سایت خرج می‌شود: فروشگاه، دوره و اشتراک Pro.
هر ماژول فروشنده همان تراکنش پرداخت درگاه را می‌نویسد، فقط طرف بدهکارش
`LedgerAccountRef::wallet(خریدار)` است نه خزانه
(`App\Support\Payments\PaymentSource`). برداشت نقدی و انتقال به کاربر دیگر
همچنان ساخته نمی‌شود.

`LedgerService::record()` ردیف کیف پول را پیش از اعمال هر ردیف با
`lockForUpdate` قفل می‌کند؛ پس دو خرید هم‌زمان از یک کیف پول نمی‌توانند هر
دو موجودی قدیم را بخوانند. بدهکارکردن بیش از موجودی به قاعده «مبلغ منفی
وجود ندارد» در `Money` می‌خورد و کل تراکنش برمی‌گردد.
