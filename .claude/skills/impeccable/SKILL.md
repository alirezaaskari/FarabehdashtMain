...
1. **This project vendors Impeccable without its launcher** (no `scripts/`, no binary, no hooks; see `.claude/skills/README.md`). Skip the launcher: read `CLAUDE.md` (its design-layer rules are this project's DESIGN.md and override anything in this skill), `resources/css/tokens.css` and `resources/css/app.css` for tokens, and the relevant Blade components, then follow the "Launcher unavailable" path below without sending its message. Any `scripts/impeccable <verb>` step in a reference (detect, live, hooks, pin, doctor, generate-image, etc.) is unavailable here: do that step by reading code and using Playwright screenshots instead.
